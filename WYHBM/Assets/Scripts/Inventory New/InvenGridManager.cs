...
public class InvenGridManager : MonoBehaviour {
    ...
    public GameObject canvasInvent;
    ...
    public bool _inInventory = false;
     ...
     private void Awake() 
     {
            canvasInvent.GetComponent<CanvasRenderer>();
     }
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.I))// Mover a PlayerController
        {
            InventoryOn();
        ...
        if (Input.GetMouseButtonUp(0) && _inInventory)
        {
    ...
    private void InventoryOn()
    {
        if (_isPausing)
        {
            canvasInvent.SetActive(true);
            Time.timeScale = 0f;
        }
        else
        {
            canvasInvent.SetActive(false);
            Time.timeScale = 1f;
        }
    }
