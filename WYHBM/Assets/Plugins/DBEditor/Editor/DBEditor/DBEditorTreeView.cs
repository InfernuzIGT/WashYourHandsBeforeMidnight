...
class DBEditorTreeView : TreeView
{
    ...
    public DBEditorTreeView(TreeViewState treeViewState, DBEditorConfig configParam) : base(treeViewState)
    {
        ...
        //_scriptableObjectIcon = AssetPreview.GetMiniTypeThumbnail(typeof(ScriptableObject)); //EditorGUIUtility.FindTexture("ScriptableObject Icon");
    ...
    protected override TreeViewItem BuildRoot()
    {
        ...
        {
            cachedCategory = root;
            var pathArray = (config.Configs[i].TreeViewName).Split('/');
            ...
            for (int j = 0; j < config.Configs[i].Files.Count; j++)
            {
                ...
                {
                    Debug.LogWarningFormat("Missing elements, alphabetical sorting not done at category {0}.", config.Configs[i].TreeViewName);
                    sort = false;
                    break;
                }
            }
            ...
            for (int j = 0; j < config.Configs[i].Files.Count; j++)
            {
                if (config.Configs[i].Files[j] == null)
                {
                    Debug.LogWarningFormat("Missing element expected in config item {0} ({1}), at index position {2}.", config.Configs[i].TreeViewName, config.Configs[i].ClassName, j);
                    continue;
                }
                ...
                {
                    Debug.LogWarningFormat("Element {0} with id {1} already exist in the database. Ignoring add at category {2}.", config.Configs[i].Files[j].name, id, config.Configs[i].TreeViewName);
                    continue;
                }
                _configIdxById.Add(id, i);
                //Debug.Log(config.files[i].GetInstanceID());
                item.displayName = config.Configs[i].Files[j].name;
                item.icon = AssetPreview.GetMiniThumbnail(config.Configs[i].Files[j]); // _scriptableObjectIcon;
