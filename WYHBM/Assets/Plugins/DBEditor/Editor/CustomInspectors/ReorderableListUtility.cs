...
public static class ReorderableListUtility
{
    ...
    public static ReorderableList CreateAutoLayout(SerializedProperty property, float?[] columnWidth)
    {
        return CreateAutoLayout(property, true, true, true, true, null, columnWidth, 10);
    }
    ...
    public static bool DoLayoutListWithFoldout(ReorderableList list)
    {
        var property = list.serializedProperty;
        property.isExpanded = EditorGUILayout.Foldout(property.isExpanded, list.count != 0 ? $"{property.displayName}: {list.count}" : property.displayName);
        // property.isExpanded = EditorGUILayout.Foldout(property.isExpanded, label != null ? label : $"{property.displayName}: {list.count}");
